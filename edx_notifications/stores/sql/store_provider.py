...
class SQLNotificationStoreProvider(BaseNotificationStoreProvider):
    ...
    def purge_notifications_for_users(self, user_ids):
        ...
        SQLNotificationMessage.objects.filter(id__in=user_notifications.values_list('msg_id', flat=True)).delete()
        SQLNotificationMessage.objects.filter(id__in=archived_notifications.values_list('msg_id', flat=True)).delete()
